          ...
          <div>
            ...
            {jobPortalFreeMode && freeModeExpires && (
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-amber-200/60 border border-amber-400 text-amber-950 text-xs font-black mb-4">
                <span>🗓️ Valid until {new Date(freeModeExpires).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' })} (Auto-reverts to Paid Mode in 30 days)</span>
              </div>
            )}
            {jobPortalFreeMode && !freeModeExpires && (
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-amber-200/60 border border-amber-400 text-amber-950 text-xs font-black mb-4">
                    ...
                    <div key={i} className="py-3 flex justify-between items-center">
                      ...
                      <div className="text-right text-xs">
                        {u.pendingDays ? (
                          ...
                          <div className="font-bold text-emerald-600">Expires: {new Date(u.jobPortalPremiumExpires).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' })}</div>
                        )}
                      </div>
                    </div>
                  ))}
                      ...
                      <td className="py-4 px-6 text-xs font-bold text-slate-600">
                        {user.isPremium && user.expiresAt ? (
                          <span className="text-amber-700 bg-amber-50 px-2 py-1 rounded border border-amber-200">
                            🗓️ {new Date(user.expiresAt).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' })}
                          </span>
                        ) : (
