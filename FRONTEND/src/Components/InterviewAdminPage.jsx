  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white rounded-2xl border border-emerald-200 p-5 flex items-center gap-4 shadow-sm">
          ...
          <div>
            ...
            <p className="text-2xl font-black text-slate-800">₹{(earnings?.totalEarnings || 0).toLocaleString('en-IN')}</p>
          </div>
        </div>
        <div className="bg-white rounded-2xl border border-blue-200 p-5 flex items-center gap-4 shadow-sm">
          <div className="w-11 h-11 rounded-xl flex items-center justify-center bg-blue-100">
            <TrendingUp className="w-5 h-5 text-blue-600" />
          </div>
          <div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Last 7 Days</p>
            <p className="text-2xl font-black text-slate-800">₹{(earnings?.last7DaysEarnings || 0).toLocaleString('en-IN')}</p>
          </div>
        </div>
      </div>
      ...
      {dailyData.length > 0 && (
        <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-5">
            ...
            <p className="text-sm font-bold text-slate-600 uppercase tracking-wider">Earnings — Last 7 Days</p>
          </div>
          <div className="flex items-end gap-2 h-32">
            {dailyData.map(([date, amount]) => {
              const barHeight = maxDay > 0 ? Math.max((amount / maxDay) * 100, amount > 0 ? 6 : 2) : 2;
              ...
              return (
                <div key={date} className="flex-1 flex flex-col items-center gap-1.5 group">
                  <div className="relative w-full flex flex-col items-center justify-end" style={{ height: '96px' }}>
                    {amount > 0 && (
                      <span className="absolute -top-5 text-[10px] font-bold text-emerald-600 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">₹{amount}</span>
                    )}
                    <div
                      className={`w-full rounded-t-lg transition-all ${amount > 0 ? 'bg-indigo-500 hover:bg-indigo-600' : 'bg-slate-100'}`}
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
          ...
          <div className="space-y-2">
            {earnings.recentPayments.map((p, i) => (
              <div key={i} className="flex items-center justify-between px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-100">
                <div>
                  <p className="text-sm font-bold text-slate-700">{p.userName}</p>
                  <p className="text-xs text-slate-400">{p.packageId?.replace(/_/g, ' ')} · {new Date(p.paidAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
                </div>
