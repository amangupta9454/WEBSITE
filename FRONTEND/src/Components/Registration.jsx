...
const Registration = () => {
  ...
  const handleResumeUpload = async (e) => {
    ...
    uploadFormData.append('upload_preset', import.meta.env.VITE_CLOUDINARY_UPLOAD_PRESET);
    uploadFormData.append('folder', 'internship-resumes');
    // Note: resource_type is automatically 'raw' when using /raw/upload endpoint
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(data.error?.message || 'Upload failed. Check console for details.');
        return;
      }
      ...
      if (data.secure_url) {
        ...
        toast.error('Upload failed: No secure_url received');
      }
    } catch (error) {
      console.error('Network or upload error:', error);
      toast.error('Resume upload failed. Check your internet connection.');
    } finally {
      setUploading(false);
    }
  };
