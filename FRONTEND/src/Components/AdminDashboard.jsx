              ...
              >
                <div>
                  ...
                  <div className="flex gap-2 mt-2">
                    ...
                    {task.deadline && (
                      <Badge variant="slate">
                        Due: {new Date(task.deadline).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' })}
                      </Badge>
                    )}
                  </div>
                </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="w-full bg-white border border-slate-300 text-slate-900 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer"
                  />
                  {bulkActionForm.startDate && (
                    <button
                      type="button"
                      onClick={() => setBulkActionForm({...bulkActionForm, startDate: ""})}
                      className="px-2.5 py-2 text-xs bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-lg border border-slate-300 whitespace-nowrap font-medium transition-colors"
                    >
                      Clear (Leave Unchanged)
                    </button>
                  )}
                </div>
                <p className="text-xs text-slate-500 mt-1">Leave empty to keep existing start dates unchanged. End date will be calculated automatically based on duration.</p>
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-2">
                  Offer Letter Status
